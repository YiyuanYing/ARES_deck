...
ui/
  ...
  action_command/       # B 键动作指令弹窗、2x3 指令和 PLACE/RELEASE button pulse
...
默认会按配置频率打印分组 debug log，包含 Joy 发布频率、UDP 接收频率、online/timeout/estop、seq、axes 和当前按下的按钮。临时关闭：
...
B 键动作窗口不再单独发送 JSON topic 或低频 action JSON。它和主界面 8 个虚拟键一样，映射成 `ControllerFrame V2` 的普通 button bit，并进入 ROS `/controller` Joy 消息：
...
| 46 | ACTION_RELEASE | `RELEASE` |
| 47 | ACTION_PLACE | `PLACE` |

点击 Target Action 弹窗按钮时，对应 button bit 会短暂置为 True；在 ROS 中表现为 `/controller.buttons[40..47]` 的短脉冲。
...
| 46 | ACTION_RELEASE | Target Action `RELEASE` |
